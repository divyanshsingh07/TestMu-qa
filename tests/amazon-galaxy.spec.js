...
async function searchAndOpenFirstProduct(page, query) {
  ...
  const listingPrice = await productLink
    .locator('xpath=ancestor::div[@data-component-type="s-search-result"][1]')
...
test.describe('Amazon - Samsung Galaxy purchase flow', () => {
  test('search Samsung Galaxy, print price, and add to cart', async ({ page }) => {
    try {
      const { productPage, listingPrice } = await searchAndOpenFirstProduct(page, 'Samsung Galaxy');

      const productPrice = (await getProductPrice(productPage)) || listingPrice || 'Price not found';
      console.log(`[Samsung Galaxy] Product price: ${productPrice}`);

      await addToCartGraceful(productPage, 'Samsung Galaxy');
    } catch (error) {
      await page.screenshot({ path: 'test-results/galaxy-failure.png', fullPage: true }).catch(() => {});
      throw new Error(`Samsung Galaxy flow failed: ${error.message}`);
    }
  });
});
